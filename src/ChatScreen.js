...
const ChatScreen = ({match}) => {
    ...
    return (
        <div className="chatDetailScreen">
            <p style={{ textAlign: "center", color: "grey", marginBottom: "20px"}}>You matched with {sender}</p>
            <div className="chat_container">
                {
                    messages.map((msg, index) => {
                        return (
                            <div key={index} className={msg.received ? "chat_left" : "chat_right"}>
                                {
                                    msg.received ? 
                                        <Avatar 
                                            className="chat_img" 
                                            alt={sender} 
                                            src={senderPicLink}/> : null
                                }
                                <p className={`chat_msg ${msg.received ? 'text_color_received' : 'text_color_send'}`}>{msg.msg}</p>
                            </div>
                        )
                    })
                }
